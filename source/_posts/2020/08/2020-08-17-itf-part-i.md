...
You can read about [different reason][background] not to use options.
...
So now we have to define the project which is used to use our plugin. As always in Apache Maven
you should follow convention over configuration and the ITF is not an exception of that.
We locate the integration itself into a location. I strongly recommend to name your 
integration tests like `*IT.java` but of course you can change that if you like.
```
.
└── src/
    └── test/
...
Part III:
 ...
 - Log output checking in different variations.
